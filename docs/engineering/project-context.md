...
| Test harness | Defined and running: `node tools/dev/php.mjs lint` and `node tools/dev/php.mjs test` execute the real PHP on a WebAssembly runtime, so "verified" means executed. Current state: 196 tests / 0 failed / 931 assertions and 87 files lint-clean; the repository stays clean after a run (`storage/` holds only `.gitkeep`). Browser-side cases live in `tests/browser/` and have two runners: `node tools/dev/browser-tests.mjs` (jsdom, executed here, 14 cases) and the development-only page `/tests/browser` in a real browser (`O-11` option 2). The product itself never needs Node. jsdom has no layout engine: anything visual is the owner's browser to judge |
