...
| Test harness | Defined and running: `node tools/dev/php.mjs lint` and `node tools/dev/php.mjs test` execute the real PHP on a WebAssembly runtime, so "verified" means executed. Current state: 191 tests / 0 failed / 898 assertions and 84 files lint-clean; the repository stays clean after a run (`storage/` holds only `.gitkeep`). The product itself never needs Node (`O-11` closed for the server side; browser-side tests arrive with the design engine in Phase 1). |
...
| `O-11` | Test tooling for PHP and for the browser, and how it is installed on a shared host | **open - options presented to the owner on 2026-09-14, awaiting a choice.** (1) Server-side PHP: already running, dependency-free (`tests/run.php`), and never needed on the host - `bin/smoke.php` and `bin/check-requirements.php` are the host-side checks. (2) Browser-side JS: a committed in-browser harness page with a tiny assert file and fixtures, needing no install, is the recommendation; Node+jsdom is rejected because it cannot see canvas/RTL/font behaviour; Playwright/puppeteer is powerful but needs a browser download (blocked in the development sandbox) and is a Phase 1+ option | Whether any claim can be automatically verified | Phase 0, every phase |
...
| Browser automation | not available, and not installable here: the browser-download hosts (`cdn.playwright.dev`, `storage.googleapis.com`) are unreachable from this sandbox - only the npm registry responds. Browser-side tests therefore have to run in the owner's browser (`O-11`) |
