...
| `O-2` | Database engine | **open** - must be confirmed from the host panel (see `O-20`) | Schema, queries, migrations, Persian collation and search ordering | Phase 0 persistence layer |
| `O-3` | PHP version available on the host (and which extensions) | **open** - read from the host panel | Language features, security support, image/JSON/mbstring availability | All backend code |
| `O-4` | Concrete host, panel type, deployment mechanics | **partially answered** by `C-8`; the actual host is `open` | Deployment steps, permissions, TLS, cron availability | Phase 0 deployment runbook |
...
| `O-6` | Authentication method | **open** - mobile + OTP via Kaveh Negar is the working assumption, password login is unconfirmed | Auth design, session model, rate limiting | Phase 4 |
...
| `O-14` | Print output contract: product print areas, sizes in cm, required DPI, colour handling, accepted file formats | **open** | The design engine's export contract and print fidelity | Phase 1, 2 |
| `O-15` | B2B model: how business accounts differ (organizations, sub-users, price lists, white-label, subdomain), and the commercial model (commission, subscription, per-order) | **open** | Multi-tenancy design, permissions, pricing | Phase 6 |
...
| `O-18` | Purchase flow shape: cart with multiple items, or single-item "buy now" as in the demo | **open** | Order model, tables, UI | Phase 3, 5 |
...
| `O-20` | Verified host capability facts: PHP version, database engine and version, cron availability, MySQL connection limits, upload/post size limits, disk quota, outgoing HTTP support, SSL | **open** - owner to read from the host panel and record here | Nearly every technical decision | Phase 0 |
