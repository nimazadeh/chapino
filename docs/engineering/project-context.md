...
| Product code | **Exists, Phase 0 in progress.** Slice 1 (core: configuration, routing, error handling, logging, Persian validation), slice 2 (data layer, migrations, installer, environment report) slice 3 (session/cookie policy, CSRF, database-backed rate limiting, database-backed job queue with a cron entry point, operator tools) and slice 4 (RTL design system with a self-hosted Persian font, base script, base components, server-side view layer, home page and a development-only style guide) are delivered. Reports: [slice 1](reports/2026-09-14-phase-0-slice-1-core.md), [slice 2](reports/2026-09-14-phase-0-slice-2-database.md), [slice 3](reports/2026-09-14-phase-0-slice-3-security-and-jobs.md), [slice 4](reports/2026-09-14-phase-0-slice-4-design-system.md). |
...
| Test harness | Defined and running: `node tools/dev/php.mjs lint` and `node tools/dev/php.mjs test` execute the real PHP on a WebAssembly runtime, so "verified" means executed. Current state: 166 tests / 0 failed / 620 assertions and 76 files lint-clean; the repository stays clean after a run (`storage/` holds only `.gitkeep`). The product itself never needs Node (`O-11` closed for the server side; browser-side tests arrive with the design engine in Phase 1). |
