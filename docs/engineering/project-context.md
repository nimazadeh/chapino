...
| Product code | **Exists, Phase 0 in progress.** Slice 1 (core: configuration, routing, error handling, logging, Persian validation), slice 2 (data layer, migrations, installer, environment report) slice 3 (session/cookie policy, CSRF, database-backed rate limiting, database-backed job queue with a cron entry point, operator tools) slice 4 (RTL design system with a self-hosted Persian font, base script, base components, server-side view layer, home page and a development-only style guide) and slice 5 (create-only settings seeder with owner-input marking, capability report with required/optional/fact lines and machine-readable output, cron heartbeat) slice 6 (a browser test harness whose cases run identically in a real browser and in jsdom, behind a development-only page) and slice 7 (the install-and-deploy runbook plus the panel-only install path: a standalone `public/install.php` for hosts without shell access, and the token-gated `/maintenance` page for later migrations and new default settings) are delivered. Reports: [slice 1](reports/2026-09-14-phase-0-slice-1-core.md), [slice 2](reports/2026-09-14-phase-0-slice-2-database.md), [slice 3](reports/2026-09-14-phase-0-slice-3-security-and-jobs.md), [slice 4](reports/2026-09-14-phase-0-slice-4-design-system.md), [slice 5](reports/2026-09-14-phase-0-slice-5-seed-and-capability-report.md), [slice 6](reports/2026-09-14-phase-0-slice-6-browser-harness.md), [slice 7](reports/2026-09-14-phase-0-slice-7-install-runbook.md), [slice 8](reports/2026-09-14-phase-0-slice-8-web-installer.md). |
| Architecture decisions | Seven ratified: [ADR-0001](adr/ADR-0001-browser-side-design-engine.md), [ADR-0002](adr/ADR-0002-shared-hosting-target.md), [ADR-0003](adr/ADR-0003-deferred-ai-provider-seam.md), [ADR-0004](adr/ADR-0004-iran-region-integrations.md), [ADR-0005](adr/ADR-0005-data-layer-and-migrations.md), [ADR-0006](adr/ADR-0006-sessions-csrf-rate-limiting-and-job-queue.md), [ADR-0007](adr/ADR-0007-settings-and-seeds.md). |
...
| Install and deployment | The runbook exists: [install-and-deploy.md](runbooks/install-and-deploy.md) - XAMPP, Laragon, `php -S`, database setup, install, verification, cron, production mode, troubleshooting, and the place to paste the real capability report (`O-20`). The document root rule (`public/` only) is enforced by `.htaccess` files and pinned by `DeploymentContractTest`. The panel-only path (hosts without shell) is **built on the owner's approval**: the web installer at `public/install.php` (one-time token from `storage/install-token.php`, CSRF, five-attempt lockout, capability report before anything is written, self-disabling once `config/config.php` exists) and, for later updates, the operator page `/maintenance` (same token, plus migration status and the create-only settings seeder). **Still missing for item 0.10:** the emergency SQL export for phpMyAdmin (section 13.4 of the runbook) and the real run on the host (`O-20`) |
...
| Application directory layout | Defined and committed: `app/` (PSR-4 `App\`, with `Views/` for server-rendered templates), `public/` (the only web-exposed directory: front controller plus `assets/` with the stylesheets, the base script and the self-hosted font), `bin/` (installer, migrate, environment report, crash-free self-check, cron, queue), `config/`, `database/migrations/`, `storage/` (runtime state, outside the web root), `tests/`, `tools/dev/` (development-only tooling). See [README](../../README.md). |
| Test harness | Defined and running: `node tools/dev/php.mjs lint` and `node tools/dev/php.mjs test` execute the real PHP on a WebAssembly runtime, so "verified" means executed. Current state: 214 tests / 0 failed / 1021 assertions and 98 files lint-clean; the repository stays clean after a run (`storage/` holds only `.gitkeep`). Browser-side cases live in `tests/browser/` and have two runners: `node tools/dev/browser-tests.mjs` (jsdom, executed here, 14 cases) and the development-only page `/tests/browser` in a real browser (`O-11` option 2). The product itself never needs Node. jsdom has no layout engine: anything visual is the owner's browser to judge |
...
| [ADR-0005](adr/ADR-0005-data-layer-and-migrations.md) | Hand-written data layer, dialect-aware schema builder, plain-PHP migrations | accepted |
| [ADR-0006](adr/ADR-0006-sessions-csrf-rate-limiting-and-job-queue.md) | PHP-native sessions with a data-backed identity, deny-by-default CSRF, database-backed rate limiting, cron-driven job queue | accepted |
| [ADR-0007](adr/ADR-0007-settings-and-seeds.md) | Config holds host decisions and credentials; the `settings` table holds operator decisions; seeds are create-only and owner-input keys stay empty | accepted |
...
| `O-10` | Traffic, data volume and concurrency expectations, and whether B2B is sold in the same offer as SaaS | **B2B shape decided by the owner on 2026-09-14: organizational accounts.** B2B is sold in the same offer as SaaS, inside the same codebase, as organizations with members, roles, a private catalogue and organization price lists (`O-15`) - not as a wholesale storefront and not as a white-label instance. Nothing about it is built before the Phase 6 gate, and pricing, onboarding or the landing page must not promise it before then. The traffic and volume half of this question is still **open**: the platform must stay usable at the smallest scale until real numbers exist (`O-20`) | Indexing, caching, architecture decisions, B2B scope | Phase 0, 6, 7 |
| `O-11` | Test tooling for PHP and for the browser, and how it is installed on a shared host | **Decided by the owner on 2026-09-14: option 2.** Server-side PHP tests stay as they are (`tests/run.php`: no install, never needed on the host - `bin/smoke.php` and `bin/check-requirements.php` are the host-side checks). Browser-side: a committed in-browser harness - the development-only page `/tests/browser` plus the shared cases in `tests/browser/cases.js` and the fixtures in `tests/browser/fixtures.html` - executed in the owner's own browser with nothing installed. The same cases run in jsdom (`node tools/dev/browser-tests.mjs`) so that a result can be recorded here; jsdom has no layout engine, so visual judgement stays with the owner. Playwright/puppeteer remains a Phase 1+ option | Whether any claim can be automatically verified | Phase 0, every phase |
...
| `O-15` | Commercial model and B2B shape | **commercial model decided (2026-09-14): hybrid** - a free base plan with commission on sales plus a paid professional subscription. B2B structural shape is **decided (2026-09-14, owner): organizational accounts** - organizations, members/sub-users, roles, a private catalogue and organization price lists. A wholesale-only storefront and white-label instances are not the chosen shape. Implementation stays a Phase 6 gate, and the pricing of organization plans is still open | Multi-tenancy design, permissions, pricing | Phase 3 (plans/quotas), Phase 6 (B2B shape) |
...
| Browser automation | No real browser exists in the development sandbox: the browser-download hosts (`cdn.playwright.dev`, `storage.googleapis.com`) are unreachable (only the npm registry responds) and the required system libraries are absent. The committed harness (`O-11`, option 2) therefore has two runners: jsdom here, which executes the cases and records a result, and the owner's browser at `/tests/browser` (development only), which is the only place layout, fonts and RTL shaping can be judged. Never report a browser-side visual result that was not seen in a browser |
| Outgoing HTTP from the sandbox | **blocked** except the npm registry. The capability report's gateway check (`curl`, 8s timeout) therefore reports a WARN here and must be re-run on the real host (`O-20`); the report classifies it as `optional`, never as a failure |
| PHP in the runner | 8.4.25 on the WebAssembly runtime; `memory_limit=256M`, `max_execution_time=0`, `intl` absent |
